...
function save_now_function(){
	...
	browser.runtime.sendMessage({message: "openurl", wayback_url: wb_url, page_url:url , method:'save' });
    console.log(url);
...
function recent_capture_function(){
	...
	browser.runtime.sendMessage({message: "openurl", wayback_url: wb_url,page_url:url, method:'recent' }, function(response) {
	});
}
...
function first_capture_function(){
	...
	browser.runtime.sendMessage({message: "openurl", wayback_url: wb_url,page_url:url, method:'first' }, function(response) {
	});
}
...
function view_all_function(){
	...
	browser.runtime.sendMessage({message: "openurl", wayback_url: wb_url,page_url:url, method:'viewall' }, function(response) {
	});
}
...
function about_support(){
  var myWindow = window.open("about:blank", "_blank", "width=500, height=400");   // Opens a new window
...
function support_function(){
  ...
  //window.open(final_support, "", "width=500, height=400");
    //browser.tabs.create({url:final_support});
...
function restoreSettings() {
  //count=0;
  browser.storage.local.get(["as"], function(items) {
    
    document.getElementById('as').checked = items["as"];  
      if(items["as"]){
          browser.runtime.sendMessage({message: "start_as"}, function(response) {});
      }
     });
}
...
function makeModal(){
    ...
    browser.runtime.sendMessage({message: "makemodal",rturl:url}, function(response) {
	});
